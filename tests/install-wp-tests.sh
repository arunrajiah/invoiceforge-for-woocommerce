#!/usr/bin/env bash
# Installs WordPress and the WP test suite into a temp directory.
#
# Usage: install-wp-tests.sh <db-name> <db-user> <db-pass> [db-host] [wp-version] [skip-database-create]
#
# Adapted from the WP-CLI scaffold plugin template.
...
if [[ $WP_VERSION =~ ^[0-9]+\.[0-9]+$ ]]; then
    ...
    if [[ $WP_VERSION =~ [0-9]+\.[0-9]+\.[0] ]]; then
        # version x.x.0 doesn't have a tag, so use the branch
        WP_TESTS_TAG="branches/${WP_VERSION%??}"
    else
    ...
    if [[ -z "$LATEST_VERSION" ]]; then
        echo "Latest WordPress version could not be found" 1>&2
        exit 1
    fi
